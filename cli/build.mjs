...
// Copy yoga.wasm file from node_modules
const yogaWasmPath = path.resolve("node_modules/yoga-wasm-web/dist/yoga.wasm");
const yogaWasmDest = path.resolve(OUT_DIR, "yoga.wasm");
if (fs.existsSync(yogaWasmPath)) {
  fs.copyFileSync(yogaWasmPath, yogaWasmDest);
} else {
  console.error("Could not find yoga.wasm file in node_modules");
  process.exit(1);
}
